              ...
              <CardTitle className="flex justify-between">
                ...
                <div className="flex gap-2">
                  {editingId === memo.id ? (
                    ...
                    <Button size="sm" variant="outline" onClick={() => startEditing(memo.id)}>편집</Button>
                  )}
                  <Button variant="destructive" size="sm" onClick={() => deleteMemo(memo.id)}>삭제</Button>
                </div>
              </CardTitle>
            </CardHeader>
            <CardContent>
              ...
              <p className="text-sm text-gray-500 mt-2">생성일: {new Date(memo.createdAt).toLocaleDateString()}</p>
            </CardContent>
          </Card>
        ))}
      </div>
